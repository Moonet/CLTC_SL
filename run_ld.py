...
from pytorch_pretrained_bert.modeling import BertForSequenceClassification, BertForMultilingualRep
...
class MLDProcessor(DataProcessor):
    ...
    def __init__(self, lang, tokenizer):
        ...
        with open('en-ja.txt', "r", encoding='utf-8') as f:
    ...
    def get_train_examples(self, data_dir):
        ...
        for (i, line) in enumerate(lines):
            ...
            new_text_a = []
            for word in text_a_list:
                if word in self.en2zh.keys():
                    new_text_a.append(self.en2zh[word])
                else:
                    new_text_a.append(word)
            # text_a_wr = ' '.join(new_text_a)
            examples.append(
                InputExample(guid=guid, text_a=text_a, label=label))
            # examples.append(
            #     InputExample(guid=guid, text_a=text_a_wr, label=label))
